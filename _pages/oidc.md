...
Login.gov supports only **private_key_jwt** for client authentication.
...
**client_secret** is not supported by Login.gov because of security reasons. It requires managing a shared secret in two places: the client and the server, where **private_key_jwt** flow involves only sharing public keys with the server.

Clients that are unable to use private_key_jwt (e.g. native mobile apps) must use **PKCE**, short for [Proof Key for Code Exchange by OAuth Public Clients](https://tools.ietf.org/html/rfc7636) and pronounced "pixy." In this method, the client sends a public identifier as well as a hashed random value generated by the client.

Other implementations of OpenID Connect use the "implicit flow", but Login.gov does not support those methods. The implicit flow is [not recommended by the OAuth group][oauth-implicit-deprecated] for security reasons.
...
* **code_challenge** — *required for PKCE*
  ...
  url_safe_code_challenge = Base64.urlsafe_encode64(code_challenge).tr('=','')
  # RFC 4648 URL-safe Base64 encoding replaces "+" with "-" and "/" with "_", with "=" trimmed
