...
token_request_schema:
  - key: client_assertion
    requirement: required for private_key_jwt
    description: >
    ...
    - key: iss
      type: string
      description: >
        The issuer, which must be the `client_id`.
    - key: sub
      type: string
      description: >
        The subject, which must also be the `client_id`.
    - key: aud
      type: string
      description: >
        The audience, which should be (or, in the case of multiple audience values, include) the URL of the token endpoint, for example: `https://idp.int.identitysandbox.gov/api/openid_connect/token`
    - key: jti
      type: string
      description: >
        The JWT ID, a unique identifier for the token which can be used to prevent reuse of the token. Should be an un-guessable, random string generated by the client.
    - key: exp
      type: number
      description: >
        The expiration time for this token. Should be an integer timestamp (number of seconds since the [Unix Epoch](https://en.wikipedia.org/wiki/Unix_time)) and be a short period of time in the future (such as 5 minutes from now).
...
id_token_schema:
  ...
  - key: sub
    type: string
    description: >
      The subject identifier, the UUID of the login.gov user (see [user attributes]({{ site.baseurl }}/attributes/)).
  - key: aud
    type: string
    description: >
  ...
  - key: acr
    type: string
    description: >
      The Authentication Context Class Reference value of the returned claims, from the original [authorization request](#authorization).
  - key: at_hash
    type: string
    description: >
  ...
  - key: nonce
    type: string
    description: >
      The nonce value provided by the client in the [authorization request](#authorization).
...
{% include schema.html schema=page.token_request_schema %}

### Token response

The token response will be a JSON object containing the following:

{% include schema.html schema=page.token_response_schema %}
...
{% include schema.html schema=page.userinfo_response_schema %}
