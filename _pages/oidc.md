...
If authenticating via JWT, you can use [OpenSSL](https://www.openssl.org) to generate a key-pair. The public key will be registered with login.gov, while the private key will be used to generate the JWT.

First create an `openssl.conf` config file which will be used during the key-generation process. Feel free to use the following `openssl.conf` example, but edit the `req_distinguished_name` section to include information about your organization and your application:
...
commonName             = YOUR_DOMAIN # a URL representing your application's domain
...
Generate the key-pair, specifying the configuration file path as well as paths where the key files should be created:

```sh
openssl req -x509 -sha256 -nodes -days 365 -newkey rsa:2048 -keyout path/to/my_app_name.key -out path/to/my_app_name.crt -config path/to/openssl.conf
```

Register the public key (`.crt`) file with login.gov. It will look something like this:
...
Use the private key when generating your JWT.
...
The configuration response contains metadata about the identity provider, including links to other endpoints. Here is an example response from [https://idp.int.login.gov/.well-known/openid-configuration](https://idp.int.login.gov/.well-known/openid-configuration):
...
```
{
  ...
  "authorization_endpoint":"https://idp.int.login.gov/openid_connect/authorize",
  "issuer":"https://idp.int.login.gov/",
  "jwks_uri":"https://idp.int.login.gov/api/openid_connect/certs",
  "service_documentation":"https://pages.18f.gov/identity-dev-docs/",
  "token_endpoint":"https://idp.int.login.gov/api/openid_connect/token",
  "userinfo_endpoint":"https://idp.int.login.gov/api/openid_connect/userinfo",
  "end_session_endpoint":"https://idp.int.login.gov/openid_connect/logout",
...
* **client_assertion** — *required for private_key_jwt*
  A [JWT][jwt] signed with the client's private key and containing the following claims:
    ...
    * **aud** (string) — The audience, which should be the URL of the token endpoint, for example: `https://idp.int.login.gov/api/openid_connect/token`
...
* **iss** (string) — The issuer of the response, which will be the URL of the login.gov IdP, for example: `https://idp.int.login.gov`
...
* **iss** (string)
  The issuer of the response, which will be the URL of the login.gov IdP, for example: `https://idp.int.login.gov`
