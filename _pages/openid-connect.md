...
<div markdown="1" data-example="pkce">
```bash
POST https://idp.int.login.gov/openid_connect/token
...
* **client_assertion** *required for `private_key_jwt`*
  ...
  The JWT should have the following claims, and must be signed with the client's private key.
    ...
    * **aud**
      Audience, the URL of the token endpoint. Should be `https://idp.int.login.gov/openid_connect/token`.
...
```
GET https://idp.int.login.gov/openid_connect/userinfo
